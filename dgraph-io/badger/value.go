...
const (
	...
	// The MSB 3 bits are for transactions.
	bitLevelDB byte = 1 << 5 // Set if the entry is Never key-value seperate whatever the ValueThreshold
...
func (vlog *valueLog) write(reqs []*request) error {
	...
	toDisk := func() error {
		...
		//the number of blocks is not corret under leveldb mode becasue some reqs aren't written
		//should be fine since it is only a log
	...
	for i := range reqs {
		...
		for j := range b.Entries {
			e := b.Entries[j]
			if (e.meta&bitLevelDB > 0){
				b.Ptrs = append(b.Ptrs, valuePointer{0,0,0,})
				continue;
			}
			var p valuePointer
